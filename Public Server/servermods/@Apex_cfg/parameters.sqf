...
_teamspeak_server = 'ts3.cjtf2.com : 9987     - Password: N/A';					// Teamspeak server address, for use with map marker, map tabs, ec. Customize this accordingly.		Example:	_teamspeak_server = 'ts3.address.com : 1234     - Password: N/A';
...
_website_url = 
[
	"'https://cjtf2.com'",														// Website URL of your website		Example:	"'https://goo.gl/7Xajd9'"
	"Our Website",																	// Button text.
	"Link 1"																		// Tooltip (text shown when mouse hovering over button).
];
_discord_server = 
[
	"'https://discord.gg/mqaUm3v'",														// Discord server (change this to yours).	Example:	"'https://goo.gl/7Xajd9'"
	"Our Discord",																	// Button text.
	"Link 2"																		// Tooltip (text shown when mouse hovering over button).
];
_arma_units_url = 
[
	"'https://units.arma3.com/unit/combinedjointtaskforce2'", 													// url path to your A3 Unit		Example:	"'https://goo.gl/7Xajd9'"
...
_staffNames = 'J. Downs (Task Force Commander), A. Attwell (Task Force Executive)';						// This text gets shown on the splash screen when player enters the game, customize as you like.
...
_arsenal = 0;											// Arsenal.		0 - Unrestricted. 1 - Use Whitelist. 2 - Use Blacklist.			Caution! Blacklist is unconfigured by default, you will have to do it. Only whitelist comes pre-configured.
...
_destroyer_respawning = 1;									// Player Spawning.		0 - None. 1 - All players will (re)spawn on the ship. 		Note: This option is overridden by  "_aircraft_carrier_respawning" option above. Jet pilots will also respawn on the carrier, even if both are available.
...
_serverCommandPassword = "'CJTF2ServerCommand'";			// Enter a server command password like this. It MUST match servercommandpassword from your server.cfg config file. ---> serverCommandPassword = "ShVQArtpGdc5aDQq"; This is important and some mission systems will not function without it.
